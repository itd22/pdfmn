...
## Responsibilities (all under `src/pdftui/`)
...
| `books_lib.py`   | `BooksSpine` — holds the entry list, picks json/yaml/db bridge by policy.   |
| `main.py`        | CLI (click) — `--tui yes/no` switch; classic one-shot mode reads `--action`, builds `BooksSpine`, calls `load`/`crawl_and_merge`/`save`/`print_names`. |
...
they all go through `BooksSpine`.
...
classDiagram
    ...
    BooksSpine --> json_bridge : policy == "json"
    BooksSpine --> db_bridge : policy == "db"
    BooksSpine --> merge : policy == "json" (in-memory merge)
    BooksSpine --> PdfCrawler : crawl_and_merge()
...
flowchart TD
    ...
    C1 --> D[BooksSpine policy=json]
    C2 --> E[BooksSpine policy=db]
...
`BooksSpine(policy=...)` is `"json"`, `"yaml"`, or `"db"`. Only `json_bridge.py`,
...
`python -m pdftui.main` (i.e. `--tui=yes`, the default) launches an
...
- **Save settings to file** writes the current policy/paths to
  `.pdftui_tui_settings.json` in the working directory. The TUI
...
- A single `TuiSession` (in `tui.py`) holds the current policy, paths, and
  the `BooksSpine` instance for the whole run. Actions read and mutate that
